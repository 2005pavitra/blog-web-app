...
const CreateBlog = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({
        text: error.response?.data?.error || "Error creating blog",
        type: "error",
      });
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Create New Post</h2>
          <Link
            to="/dashboard"
          ...
          <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6">
            ...
            <div className="pt-4">
              <button
                type="submit"
                disabled={loading}
                className="w-full sm:w-auto px-8 py-3 bg-slate-900 text-white font-medium rounded-lg hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-900 disabled:opacity-70 disabled:cursor-not-allowed transition-all flex items-center justify-center"
              >
                {loading ? (
                  <>
                    <svg
